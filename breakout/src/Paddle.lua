...
function Paddle:setSize(size)
    -- Calculate the difference in width between the new and current size
    local oldWidth = self.width
    local newWidth = size * 32  -- Assuming each size increment corresponds to 32 pixels in width

    -- Update the size and width of the paddle
    self.size = size
    self.width = newWidth

    -- Adjust the x position to keep the paddle centered around its previous position
    -- The idea is to offset the x position by half the difference in width
    self.x = self.x + (oldWidth - newWidth) / 2

    -- Ensure the paddle doesn't go outside the bounds of the screen
