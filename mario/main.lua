--[[
    ...
    A classic platxlformer in the style of Super Mario Bros., using a free
    ...
    jumping up into blocks typi ally breaks them or reveals a powerup.
...
local accum = 0.0
local updateRate = 1/60 -- Targeting 60 updates per second

function love.update(dt)
    accum = accum + dt
    while accum >= updateRate do
        -- Update game state considering the fixed timestep
        gStateMachine:update(updateRate)
        
        -- Reset keys pressed after processing update to ensure correct input handling
        love.keyboard.keysPressed = {}
